...
d = {p1: 78}
d[p1] ---> 78
d[p2] ---> 78 # p1 and p2 have the same keys
```
*How does python calls hash*
- In reality: 
  - hash(p)
    - looks for p.\_\_hash__
      - None means not hashable
      - otherwise, calls p.\_\_hash__
      - truncates returned integer to 32/64 bits depending on your specific python
  - import sys
  - sys.hash_infor.width -----> 32/64
- How does python truncate to that number ?
  - sys.hash_info.modulus 
    - p.\_\_hash__ % sys.hash_infor.modulus
  - hash(p) --> p.\_\_hash__() % sys.hash_info.modulus