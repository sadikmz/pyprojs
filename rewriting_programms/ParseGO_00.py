...
parser = argparse.ArgumentParser(
    description='Parse go relatin OBO file and create go relation and go gene database files', add_help=False)
...
class ParseOBO(object):
    ...
    def parseOBO(self, infile, outf, relation_file):
        with open(infile, 'r') as in_f:
            ...
            outf.write("\t".join(["go_id", "description", "name_space"]) + "\n")
            info = []
            go = ""
            for row in in_f:
                row = row.strip("\n")
                ...
                if inf[0] == "is_a":
                    ...
                    try:
                        ...
                        pass
                if row == "[Typedef]":
                    '''Stop reading file when trailing typedefinitions are explained, no more terms exists after these'''
                    self._writeRow(info, outf)
                    break
            outf.close()
        return
...
if __name__ == "__main__":
    '''
            Parse the obo file to retrieve the nessesary GO information for each term and the go relation file.
    '''
    parseOBO = ParseOBO()
